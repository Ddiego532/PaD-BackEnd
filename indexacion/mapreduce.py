...
def ejecutar_comando():
    try:
        ...
        comando_json = subprocess.run(["python", "json_to_txt.py"], check=True)
        
        # Ejecutar el script Map.py y pasar la salida como entrada al script Reduce.py
        comando_map = subprocess.Popen(["python", "Map.py"], stdout=subprocess.PIPE)
        
        comando_reduce = subprocess.Popen(["python", "Reduce.py"], stdin=comando_map.stdout, stdout=subprocess.PIPE)
